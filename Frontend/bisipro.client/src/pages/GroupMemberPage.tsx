...
import { groupMemberService, type GroupMember } from "@/services/groupMemberService"
import { groupService, type GroupDropdownItem } from "@/services/groupService"
...
const GroupMemberPage = () => {
  ...
  const [sortBy, setSortBy] = useState<string>("memberName")
  ...
  const [groups, setGroups] = useState<GroupDropdownItem[]>([])
  ...
  const fetchMembers = useCallback(async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await groupMemberService.getGroupMembers({
      ...
      setError(err?.message || "An unexpected error occurred while fetching group members.")
    } finally {
      setLoading(false)
    }
  }, [debouncedSearch, groupId, isActive, pageNumber, pageSize])
  ...
  // Load the group dropdown once to power the group filter and resolve group names
  useEffect(() => {
    let cancelled = false
    const loadGroups = async () => {
      try {
        const response = await groupService.getGroupDropdown()
        if (!cancelled && response?.isSuccess) {
          setGroups(response.data || [])
        }
      } catch (err) {
        console.error("Error fetching group dropdown:", err)
      }
    }
    loadGroups()
    return () => {
  ...
  const groupNameById = useMemo(() => {
    ...
    groups.forEach((g) => map.set(g.id, g.groupName))
    return map
  }, [groups])

  // The API does not support sorting, so ordering is applied to the current page only
  const sortedMembers = useMemo(() => {
    ...
    rows.sort((a, b) => {
      ...
      switch (sortBy) {
        case "payableAmount":
          result = a.payableAmount - b.payableAmount
          break
        case "joinedDate":
          result = new Date(a.joinedDate).getTime() - new Date(b.joinedDate).getTime()
          break
        case "isActive":
          result = Number(a.isActive) - Number(b.isActive)
          break
        case "phoneNumber":
          result = (a.phoneNumber || "").localeCompare(b.phoneNumber || "")
          break
        default:
  ...
  const pagePayableTotal = useMemo(
    () => members.reduce((sum, m) => sum + (m.payableAmount || 0), 0),
    [members]
  )

  const hasFilters = Boolean(search) || groupId !== undefined || isActive !== undefined
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[#60736c]">
            Review member enrollment, payable contributions, and participation status across your savings groups.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2 sm:self-center">
          <span className="inline-flex items-center gap-1.5 rounded-xl border border-[#c3e4ba]/40 bg-[#e2f1df]/60 px-3 py-2 text-xs font-semibold text-[#056c5c]">
          ...
          <span className="inline-flex items-center gap-1.5 rounded-xl border border-[#cedbd3] bg-white px-3 py-2 text-xs font-semibold text-[#29463f]">
            <span className="size-1.5 rounded-full bg-[#078a76]" />
            <span>{activeCount} Active on page</span>
          </span>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center">
          ...
          <div className="relative w-full md:w-44">
            ...
            >
              ...
              <option value="false">Exited Members</option>
            </select>
            <ChevronDown className="absolute top-3 right-3 size-4 text-[#788b83] pointer-events-none" />
          </div>
      ...
      {error ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-red-200 bg-red-50/50 p-8 text-center text-red-800">
          ...
          <h3 className="text-lg font-bold">Failed to Load Group Members</h3>
          ...
          <div className="hidden md:block rounded-xl border border-brand bg-white overflow-hidden shadow-sm">
            <Table>
              <TableHeader className="bg-[#f5f7f3]/50">
                <TableRow>
                  <TableHead className="w-[260px]"><Skeleton className="h-4 w-24" /></TableHead>
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-[#cedbd3] bg-white p-12 text-center shadow-sm">
          ...
          <h3 className="text-lg font-bold text-brand-ink">No members found</h3>
          <p className="mt-1 text-sm text-[#60736c] max-w-sm">
            {hasFilters
              ? "We couldn't find any members matching your filter parameters. Try resetting your search filters."
              : "There are currently no members enrolled in this workspace. Add members to a group to see them here."}
          </p>
          {hasFilters && (
              ...
              <TableHeader className="bg-[#f5f7f3]/50">
                <TableRow>
                  <TableHead
                    ...
                    className="cursor-pointer font-semibold text-brand-ink hover:bg-[#f5f7f3]/70 transition-colors w-[260px]"
                  >
                  ...
                  <TableHead className="font-semibold text-brand-ink">Group</TableHead>
                  <TableHead
                  ...
                  <TableHead className="font-semibold text-brand-ink">Exit Date</TableHead>
                  <TableHead
                ...
                {sortedMembers.map((member) => (
                  ...
                  >
                    <TableCell className="py-3.5">
                      <div className="flex items-center gap-3">
                        <span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-[#e2f1df]/70 text-xs font-bold text-[#056c5c] border border-[#c3e4ba]/40">
                          {getInitials(member.memberName)}
                        </span>
                        <div className="min-w-0">
                          <div
                            className="font-semibold text-[#183630] truncate max-w-[180px]"
                            title={member.memberName}
                          >
                            {member.memberName}
                          </div>
                          <div className="text-xs text-[#788b83] font-normal truncate max-w-[180px]">
                            Member ID · {member.userId.slice(0, 8)}
                          </div>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell className="text-[#29463f] font-medium">
                    ...
                    <TableCell className="text-[#60736c]">
                      <div
                        ...
                        title={groupNameById.get(member.groupId) ?? member.groupId}
                      >
                        {groupNameById.get(member.groupId) ?? "-"}
                      </div>
                    </TableCell>
                    <TableCell className="text-right font-medium pr-6 text-[#183630]">
                      {formatCurrency(member.payableAmount)}
                    </TableCell>
                    <TableCell className="text-[#60736c]">{formatDate(member.joinedDate)}</TableCell>
                    <TableCell className="text-[#60736c]">
              ...
              >
                <div className="flex justify-between items-start gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-[#e2f1df]/70 text-xs font-bold text-[#056c5c] border border-[#c3e4ba]/40">
                      {getInitials(member.memberName)}
                    </span>
                    <div className="min-w-0">
                      <h3 className="font-bold text-[#183630] text-base leading-tight truncate">
                        {member.memberName}
                      </h3>
                      <p className="text-xs text-[#788b83] mt-0.5 flex items-center gap-1">
                ...
                <div className="flex flex-wrap gap-2">
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold border bg-[#f5f7f3] text-[#29463f] border-[#cedbd3]">
                    <Users className="size-3" />
                    <span className="truncate max-w-[160px]">
                      {groupNameById.get(member.groupId) ?? "Group"}
                    </span>
                  </span>
                  <span
                    className={cn(
                ...
                <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-xs">
                  <div>
                    <span className="text-[#788b83] block">Payable Amount</span>
                    <span className="font-bold text-[#183630]">
                      {formatCurrency(member.payableAmount)}
                    </span>
                  </div>
                  <div>
                    <span className="text-[#788b83] block">Joined Date</span>
                    <span className="font-semibold text-[#183630]">
                      {formatDate(member.joinedDate)}
                    </span>
                  </div>
                  <div>
                    <span className="text-[#788b83] block">Exit Date</span>
                    <span className="font-semibold text-[#183630]">
                      {member.exitDate ? formatDate(member.exitDate) : "-"}
                    </span>
                  </div>
                  <div>
                    <span className="text-[#788b83] block">Status</span>
                    <span className="font-bold text-[#183630]">
                      {member.isActive ? "Active" : "Exited"}
                    </span>
                  </div>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between rounded-xl border border-brand bg-white p-4 shadow-sm">
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs sm:text-sm text-[#60736c] justify-between sm:justify-start">
              <span>
                ...
                <span className="font-semibold text-brand-ink">{totalCount}</span> members
              </span>

              <span className="hidden sm:inline">
                Page total:{" "}
                <span className="font-semibold text-brand-ink">
                  {formatCurrency(pagePayableTotal)}
                </span>
              </span>
