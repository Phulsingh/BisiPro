...
import { AlertTriangle, ArrowLeft, CalendarDays, CalendarRange, CheckCircle2, CircleDollarSign, Clock3, Gavel, Handshake, RefreshCw, Repeat2, Trophy, UserRoundPlus, Users, Wallet } from "lucide-react"
import { AgentCommandDropdown } from "@/components/AgentCommandDropdown"
...
const GroupDetailsPage = () => {
  ...
  const existingAgentIds = useMemo(() => {
    ...
    const ids = group.agentIds || (group.agentId ? [group.agentId] : [])
    return ids.filter((id) => id && id !== "00000000-0000-0000-0000-000000000000")
  }, [group])
  ...
  useEffect(() => {
    if (!assignmentDialogOpen || !group) return

    let cancelled = false
    setSelectedAgentIds(existingAgentIds)
    setAssignmentError(null)
    setLoadingAssignedAgents(true)

    const loadAssignedAgentIds = async () => {
      try {
        const response = await userService.getAssignedAgentIds(group.groupId)
        if (cancelled) return

        if (!response?.isSuccess) {
          setAssignmentError(response?.error || "Unable to load assigned agents.")
          return
        }

        setSelectedAgentIds(response.data || [])
      } catch (err: any) {
        if (cancelled) return
        console.error("Error fetching assigned agents:", err)
        setAssignmentError(err?.message || "Unable to load assigned agents.")
      } finally {
        if (!cancelled) setLoadingAssignedAgents(false)
      }
    }

    void loadAssignedAgentIds()
    return () => { cancelled = true }
  }, [assignmentDialogOpen, existingAgentIds, group])
  ...
  const saveAgentAssignment = async () => {
    ...
    try {
      ...
      setAssignmentError(null)
      const response = await groupService.assignAgents(group.groupId, { agentIds: selectedAgentIds })

      if (!response?.isSuccess) {
        setAssignmentError(response?.error || "Unable to update agent assignments.")
        return
      }

      setAssignmentDialogOpen(false)
      setAssignmentNotice("Agent assignments updated.")
      void fetchGroup()
    } catch (err: any) {
      console.error("Error assigning agents:", err)
      setAssignmentError(err?.message || "Unable to update agent assignments.")
    } finally {
  ...
  return <div className="space-y-6"><BackButton onClick={backToGroups} />
    ...
    <Dialog open={assignmentDialogOpen} onOpenChange={setAssignmentDialogOpen}>
      {assignmentDialogOpen && <DialogContent className="max-w-[calc(100%-2rem)] gap-0 overflow-hidden p-0 sm:max-w-xl">
        <DialogHeader className="border-b border-[#e4ebe6] px-6 pt-6 pb-5">
          <DialogTitle className="text-lg font-bold text-[#183630]">Assign agents</DialogTitle>
          <DialogDescription className="leading-5 text-[#60736c]">Choose the agents who can manage {group.groupName}. Selected agents are kept checked when you reopen this group.</DialogDescription>
        </DialogHeader>
        <div className="px-6 py-5">
          <div className="mb-3 flex items-center justify-between"><span className="text-sm font-semibold text-[#29463f]">Available agents</span><span className="rounded-full bg-[#e2f1df] px-2.5 py-1 text-xs font-bold text-[#056c5c]">{selectedAgentIds.length} selected</span></div>
          <AgentCommandDropdown value={selectedAgentIds} onValueChange={setSelectedAgentIds} disabled={assigningAgents || loadingAssignedAgents} />
          {assignmentError && <p className="mt-3 text-sm font-medium text-red-600" role="alert">{assignmentError}</p>}
        </div>
        <DialogFooter className="m-0 flex-row items-center justify-end gap-3 border-[#e4ebe6] bg-[#f8faf8] px-6 py-4">
          <Button type="button" variant="outline" onClick={() => setAssignmentDialogOpen(false)} disabled={assigningAgents} className="h-10 cursor-pointer rounded-xl border-[#cedbd3] px-4 text-[#29463f] hover:bg-[#edf3ee]">Cancel</Button>
          <Button type="button" onClick={saveAgentAssignment} disabled={assigningAgents || loadingAssignedAgents} className="h-10 cursor-pointer gap-2 rounded-xl bg-[#078a76] px-4 font-semibold text-white hover:bg-[#056c5c]">{(assigningAgents || loadingAssignedAgents) && <RefreshCw className="size-4 animate-spin" />}{assigningAgents ? "Saving…" : loadingAssignedAgents ? "Loading…" : "Save assignment"}</Button>
        </DialogFooter>
      </DialogContent>}
    </Dialog>
  </div>
